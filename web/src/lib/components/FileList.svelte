			...
			{#each $files as file, index (file.id)}
				<li class="group relative mb-2 flex items-center">
					<button
						...
						class="group-hover:bg-base-200 rounded-field relative min-w-0 flex-1 cursor-pointer p-2 transition-colors duration-200 { $fileSelected === index ? 'bg-base-200' : '' } { !file.uploaded && file.progress === 0 ? 'animate-pulse' : '' }"
					>
						{#if file.progress > 0}
							<div
						...
						<div class="relative flex items-center">
							<div
								class="rounded-selector mr-3 flex h-9 w-12 shrink-0 items-center justify-center { !file.thumb ? 'bg-base-200' : '' }"
