...
class TaskQueue {
	...
	private constructor() {
		log.info(
			`Task Queue initialized with max ${MAX_CONCURRENT_TASKS} concurrent tasks`,
		);
	}
	...
	public addTask(taskId: string): void {
		...
		log.info(
			`Task ${taskId} added to queue. Queue stats: ${this.getStatsString()}`,
		);
		this.processQueue();
	}
	...
	public removeTask(taskId: string): void {
		...
		log.info(
			`Task ${taskId} finished execution. Queue stats: ${this.getStatsString()}`,
		);
		this.processQueue();
	}
	...
	private async processQueue(): Promise<void> {
		...
		if (this.running.size < MAX_CONCURRENT_TASKS && this.queue.length > 0) {
			...
			log.info(
				`Processing queue: starting ${tasksToStart} tasks, ${this.queue.length - tasksToStart} will remain queued`,
			);
			...
			for (let i = 0; i < tasksToStart; i++) {
				...
				log.info(
					`Starting task ${taskId}. Queue stats: ${this.getStatsString()}`,
				);
				...
				taskService
					...
					.catch((err) => {
						log.error(
							`Task ${taskId} execution failed: ${err.message || String(err)}`,
						);
					})
					.finally(() => {
			...
			log.info(
				`Queue processing deferred: ${this.running.size}/${MAX_CONCURRENT_TASKS} tasks running, ${this.queue.length} tasks waiting`,
			);
		}
	}
